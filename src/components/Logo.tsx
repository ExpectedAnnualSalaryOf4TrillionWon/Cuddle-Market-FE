...
export default function Logo({ logoClassname, textClassname }: LogoProps) {
  return (
    <Link to={ROUTES.HOME} className="flex items-center gap-2">
      <div className={cn('h-14 w-auto object-contain md:h-16', logoClassname)}>
        <img src={CuddleMarketLogo} alt="CuddleMarket 로고" className="h-full w-full object-cover" />
      </div>

      <p className={cn('logo hidden flex-col text-2xl leading-none text-white md:flex', textClassname)}>
        <span>CUDDLE</span>
        <span>MARKET</span>
      </p>
    </Link>
